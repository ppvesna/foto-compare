...
class _StartScreenState extends State<StartScreen>
      ...
      XpMenuBar(
        ...
        menus: [
          XpMenu(label: 'Файл', items: [
            ...
            XpMenuItem(
              label: 'Зарегистрироваться', icon: '📝',
              onTap: () => setState(() => _isLogin = false),
            ),
            XpMenuItem.sep,
            XpMenuItem(
              label: 'Забыли пароль?', icon: '🔑',
              ...
              children: [
                ...
                Center(
                  child: Column(children: [
                    ...
                    const Text(
                      'PHOTO COMPARE — COLOR ANALYSIS SYSTEM',
                      style: TextStyle(
  ...
  Widget _loginForm() {
    return Column(children: [
      ...
      XpInput(placeholder: 'user@example.com',
  ...
  Widget _registerForm() {
    return Column(children: [
      ...
      const Align(alignment: Alignment.centerLeft,
          child: Text('Пароль:', style: TextStyle(fontSize: 11))),
      const SizedBox(height: 3),
      XpInput(placeholder: 'Минимум 8 символов',
          obscure: true, controller: _passCtrl),
      const SizedBox(height: 8),
      const Align(alignment: Alignment.centerLeft,
          child: Text('Подтвердите пароль:', style: TextStyle(fontSize: 11))),
      const SizedBox(height: 3),
      XpInput(placeholder: 'Повторите пароль',
          obscure: true, controller: _pass2Ctrl),
      const Divider(),
      Row(mainAxisAlignment: MainAxisAlignment.end, children: [
        XpBtn(label: 'Создать аккаунт →', primary: true,
  ...
  void _doLogin() {
    // TODO: подключить AuthApiService
    // Пока переключаем на вкладку Сравнение через MainShell
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Вход выполнен'), duration: Duration(seconds: 1)),
    );
  }
  ...
  void _doRegister() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Аккаунт создан'), duration: Duration(seconds: 1)),
    );
  }
}
...
class _KanjiBg extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const text =
        '色光比較視覚認識赤緑青黄黒白紫橙色彩鮮明対比分析画像写真比較認識処理視覚光学' * 30;
    return Padding(
...
class _PrismPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final entry = Offset(
      (apex.dx + baseL.dx) / 2, (apex.dy + baseL.dy) / 2);
    final exit = Offset(
      (apex.dx + baseR.dx) / 2, (apex.dy + baseR.dy) / 2);
    ...
    // Точки
    canvas.drawCircle(entry, 3, Paint()
      ..color = Colors.white.withOpacity(0.7)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2));
    canvas.drawCircle(exit, 4, Paint()
